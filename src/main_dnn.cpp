...
{
    const std::string config_path = "/workspace/src/config.yaml";
    ConfigProxy config = ConfigProxy(config_path);
    if (config.getBool("debug_log")) 
    {
    ...
    spdlog::set_pattern("%C-%M-%d|%T.%f|%t|[%s %! %#]%l: %v");
    // -----------------------------------------------------------------------------------------------------
    ...
    Scalar mean = cv::Scalar(0,0,0);
    bool swapRB = config.getBool("rgb");
    ...
    // Load a model.
    Net net = readNet(modelPath, configPath, config.getString("framework"));
    int backend = config.getInt("backend");
    net.setPreferableBackend(backend);
    net.setPreferableTarget(config.getInt("target"));
    ...
    VideoCapture cap;
    cap.open(config.getString("input"));
    // if (parser.has("input"))
...
void drawPred(int classId, float conf, int left, int top, int right, int bottom, Mat& frame)
{
    ...
    putText(frame, label, Point(left, top), FONT_HERSHEY_SIMPLEX, 1, Scalar());
}
